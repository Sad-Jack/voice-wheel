...
| B2 | God-класс `settings.py` — дробить инкрементально | 🟠 P2 | ◑ (Фаза 1 ✅ построители вкладок; Фаза 2 ✅ чистая логика в модули; глубже — mixin'ы — не окупается, см. ниже) |
...
**Фаза 2 ✅ (частично — выносим чистую логику в модули):** `_format_log_line` (~35 стр
презентации ленты «Логи») вынесена в `hostos/macos/log_format.py` — **без AppKit**, юнит-
тестируется напрямую (`tests/test_log_format.py`, 7 тестов, 0.01с, без окна). Поведение
идентично.

**Глубже (mixin-разбор концернов) — НЕ окупается, остановились.** Проспайкал PyObjC:
- **селекторы НЕ регистрируются из plain-mixin'а** (`unrecognized selector` при target-action) →
  при выносе концерна в mixin селекторы обязаны остаться на `NSObject` (фрагментация
  «селектор тут / хелпер там»);
- помощники-`@objc.python_method` в mixin композируются нормально, НО ссылаются на модульные
  константы `settings.py` (W/H/`_BACKEND_TO_KEY`/`LLM_BACKENDS`/…) → **circular import** без
  отдельного base-модуля.
Чистый разбор потребовал бы сперва вынести базу (константы + `_TabBuilder` + `_FlippedView`) в
`_settings_base.py`, затем концерн-mixin'ы с колбэками — это «плумбинг» с риском выше выигрыша.
Основной структурный выигрыш уже взят Фазой 1 (`_build` 440→~100). Дальше — только точечный
вынос **чистой** логики (как log_format), по мере необходимости.
