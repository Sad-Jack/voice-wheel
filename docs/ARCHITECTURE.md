...
### B3 — Пустой транскрипт зовёт LLM 🟡 P3 ⬜
`pipeline.run` на ring `transform`/`context` вызывает LLM даже при **пустом** транскрипте
(тишина/не распозналось) — лишний вызов модели на холостой нажим. Реальный LLM по BASE-правилу
вернёт пустую строку, так что криминала нет, но можно короткозамкнуть: пустой транскрипт →
не ходить в LLM. Низкий приоритет (нашли в §4 `TEST-REPORT-2026-06-01.md`).
