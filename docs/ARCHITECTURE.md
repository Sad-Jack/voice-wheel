...
src/voice_wheel/
  ...
  hostos/macos/          всё PyObjC (UI/IO под macOS) — пакет назван hostos, чтобы не тенить stdlib `platform`
    ...
    settings.py          окно настроек (NSWindow, 9 вкладок) → пишет config.json/.env  ⚠️ 2231 стр
...
| B2 | God-класс `settings.py` (2231 стр) — инкрементально дробить | 🟠 P2 | ⬜ (отложено, см. ниже) |
...
### B2 — God-класс `settings.py` 🟠 P2 ⬜ (рекомендация, НЕ делаем сейчас)
**Проблема:** один `NSObject`-подкласс `SettingsWindow` разросся до **2231 строки** — он строит
все **9 вкладок**, держит dirty-модель, управление ключами/статусом, правила-оверрайды, логику
Ollama, capture клавиш, логи. Самый крупный файл проекта; правки в нём дороги по риску.
**Почему откладываем (а не дробим сейчас):**
- Билдеры вкладок завязаны на **вложенные замыкания** внутри `_build` (`T`, `label`, `hint`,
  `row`, `popup`, `stack[0]`) — их нельзя просто вынести без передачи этих хелперов.
- PyObjC-селекторы/делегаты (`save_`, `tabView:didSelect…`, `controlTextDidChange_`,
  `windowWillClose_`) должны жить на **одном** объекте — наивное дробление на классы ломает
  wiring (конфликты метаклассов NSObject при множественном наследовании).
- Класс **рабочий и покрыт** headless-харнессом (16/16 — все вкладки строятся и реагируют).
**Безопасный первый шаг, когда дойдут руки:** вынести построители вкладок в методы
`_build_<tab>(self, ui)`, где `ui` — маленький объект с общими хелперами (`label/hint/row/popup`);
`_build` становится тонким оркестратором «создай хелперы → собери 9 вкладок». Чисто внутренняя
реорганизация без смены селекторов/поведения. Дальше — по желанию мягкие mixin'ы по концернам
(Keys/Logs/Rules), каждый со своей проверкой. Делать **только** под реальный тест-цикл в UI.
